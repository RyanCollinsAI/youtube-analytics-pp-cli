...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  youtube-analytics-pp-cli doctor
  youtube-analytics-pp-cli doctor --json
  youtube-analytics-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				hasRefreshCreds := cfg.RefreshToken != "" && cfg.ClientID != "" && cfg.ClientSecret != ""
				if header == "" && !hasRefreshCreds {
					report["auth"] = "not configured"
					report["auth_hint"] = "set YOUTUBE_CLIENT_ID, YOUTUBE_CLIENT_SECRET, and YOUTUBE_REFRESH_TOKEN, or run 'youtube-analytics-pp-cli auth login'"
					report["auth_docs_url"] = "https://developers.google.com/youtube/analytics"
				} else {
					authConfigured = true
			...
			switch {
			case os.Getenv("YOUTUBE_REFRESH_TOKEN") != "":
				authEnvSet = append(authEnvSet, "YOUTUBE_REFRESH_TOKEN")
			case os.Getenv("YOUTUBE_ACCESS_TOKEN") != "":
				authEnvSet = append(authEnvSet, "YOUTUBE_ACCESS_TOKEN")
			case os.Getenv("YOUTUBE_ANALYTICS_OAUTH2C") != "":
				authEnvSet = append(authEnvSet, "YOUTUBE_ANALYTICS_OAUTH2C")
			case authConfigured:
			...
			default:
				authEnvRequiredMissing = append(authEnvRequiredMissing, "YOUTUBE_REFRESH_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'youtube-analytics-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'youtube-analytics-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'youtube-analytics-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'youtube-analytics-pp-cli sync' to refresh."
	}
	return report
}
