...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".youtube-analytics-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("YOUTUBE_ANALYTICS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("YOUTUBE_ANALYTICS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "youtube-analytics-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.youtube-analytics-pp-cli/feedback.jsonl.
When ` + "`YOUTUBE_ANALYTICS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`YOUTUBE_ANALYTICS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  youtube-analytics-pp-cli feedback list
  youtube-analytics-pp-cli feedback list --limit 5
  youtube-analytics-pp-cli feedback list --json`,
