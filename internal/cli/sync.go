...
// syncReportMetrics is the full metric set sync requests per channel-day.
const syncReportCoreMetrics = "views,estimatedMinutesWatched,averageViewDuration,subscribersGained,subscribersLost,likes,comments,shares"

// syncReportRevenueMetrics is best-effort: a channel without the monetary
// scope 403s on these, so sync retries without them.
const syncReportRevenueMetrics = "estimatedRevenue,estimatedAdRevenue,grossRevenue,cpm,monetizedPlaybacks"
...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "sync",
		Short: "Sync daily YouTube Analytics rows into the local SQLite archive",
		Long: `Sync pulls day-dimensioned YouTube Analytics rows for each registered
channel into a local SQLite archive. Once synced, the report, rollup,
whatchanged, anomaly, rpm, and sql commands work offline against history.

Re-running sync re-fetches the trailing window and overwrites on conflict,
so late-finalized revenue and view adjustments are absorbed automatically.

Revenue metrics are best-effort: if a channel lacks the monetary OAuth
scope the API returns 403 and sync retries that channel with core metrics
only, so non-monetized channels still archive their core data.`,
		Example: `  # Sync every registered channel, last 90 days
  youtube-analytics-pp-cli sync

  # Sync a single channel, last 30 days
  youtube-analytics-pp-cli sync --channel ScrollCore --days 30

  # Sync two channels by name
  youtube-analytics-pp-cli sync --channel ScrollCore --channel CuriousCore`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// --dry-run short-circuits before any channel resolution or archive
			// access so the documented examples verify cleanly even with no
			// channels registered.
			if flags.dryRun {
			...
			if len(channelArgs) == 0 {
				for _, ch := range a.listChannels() {
					targets = append(targets, target{name: ch.Name, id: ch.ChannelID})
				}
				if len(targets) == 0 {
					return usageErr(fmt.Errorf("no channels registered; run 'channels add --name <name> --channel-id UC...' or pass --channel"))
				}
			} else {
				for _, arg := range channelArgs {
					if channelIDRE.MatchString(arg) {
						targets = append(targets, target{name: arg, id: arg})
						continue
					}
					id, ok := a.lookupChannel(arg)
					if !ok {
						return notFoundErr(fmt.Errorf("channel %q not registered; run 'channels add' first", arg))
					}
					targets = append(targets, target{name: arg, id: id})
				}
			}
			...
			for _, t := range targets {
				metrics := syncReportCoreMetrics + "," + syncReportRevenueMetrics
				raw, qerr := runReportQuery(c, t.id, reportParams{
					StartDate:  startDate,
					EndDate:    endDate,
					Metrics:    metrics,
					Dimensions: "day",
				...
				if qerr != nil && strings.Contains(qerr.Error(), "HTTP 403") {
					// Retry with core metrics only — channel likely lacks the
					// monetary scope.
					metrics = syncReportCoreMetrics
					raw, qerr = runReportQuery(c, t.id, reportParams{
						StartDate:  startDate,
						EndDate:    endDate,
						Metrics:    metrics,
				...
				if qerr != nil {
					failed++
					fmt.Fprintln(cmd.OutOrStdout(), syncErrorJSON("report", t.name, qerr))
					continue
				}

				headers, rows, derr := decodeReportRows(raw)
				if derr != nil {
					failed++
					fmt.Fprintln(cmd.OutOrStdout(), syncErrorJSON("report", t.name, derr))
					continue
				}
				...
				for _, row := range rows {
					...
					for _, h := range headers {
						...
						cells = append(cells, reportCell{Day: day, Metric: h, Value: v})
					}
				}
				if err := a.bulkUpsertReportRows(t.name, t.id, cells); err != nil {
					failed++
					fmt.Fprintln(cmd.OutOrStdout(), syncErrorJSON("report", t.name, err))
					continue
				}
				...
				fmt.Fprintln(cmd.OutOrStdout(), string(prog))
			}
			...
			if failed > 0 && failed == len(targets) {
				return apiErr(fmt.Errorf("all %d channel(s) failed to sync", failed))
			}
			return nil
		},
	}

	cmd.Flags().StringSliceVar(&channelArgs, "channel", nil, "Channel name or UC... id to sync (repeatable; empty = all registered)")
	cmd.Flags().IntVar(&days, "days", 90, "How many trailing days to sync")
