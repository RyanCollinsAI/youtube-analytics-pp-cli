...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      or run 'youtube-analytics-pp-cli auth login --client-id <id> --client-secret <secret>'."+
			"\n      Run 'youtube-analytics-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: your access token is missing or expired."+
			"\n      Set YOUTUBE_CLIENT_ID, YOUTUBE_CLIENT_SECRET, and YOUTUBE_REFRESH_TOKEN,"+
			"\n      or run 'youtube-analytics-pp-cli auth login --client-id <id> --client-secret <secret>'."+
			"\n      Run 'youtube-analytics-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your credentials are valid but lack access to this resource."+
			"\n      Revenue metrics need the yt-analytics-monetary.readonly scope on your refresh token."+
			"\n      Re-run 'youtube-analytics-pp-cli auth login' to grant the required scopes,"+
			"\n      or refresh YOUTUBE_REFRESH_TOKEN with the correct scope."+
			"\n      Run 'youtube-analytics-pp-cli doctor' to check auth status.", err))
