...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  youtube-analytics-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  youtube-analytics-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  youtube-analytics-pp-cli workflow status

  # Show status as JSON
  youtube-analytics-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("youtube-analytics-pp-cli")
			}
