...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "youtube-analytics-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "youtube-analytics-pp-cli/v2")
		}
...
func (c *Client) authHeader() (string, error) {
	...
	hasCreds := c.Config.RefreshToken != "" && c.Config.ClientID != "" && c.Config.ClientSecret != ""
	accessUsable := c.Config.AccessToken != "" &&
		(c.Config.TokenExpiry.IsZero() || time.Now().Before(c.Config.TokenExpiry))
	if hasCreds && !accessUsable {
