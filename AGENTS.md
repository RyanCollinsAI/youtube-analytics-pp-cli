# YouTube Analytics Printed CLI Agent Guide

This directory is a generated `youtube-analytics-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
youtube-analytics-pp-cli doctor --json
youtube-analytics-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
youtube-analytics-pp-cli which "<capability>" --json
youtube-analytics-pp-cli <command> --help
```
...
```bash
youtube-analytics-pp-cli <command> --agent
```
...
```bash
youtube-analytics-pp-cli <command> --help
youtube-analytics-pp-cli <command> --dry-run --agent
```
